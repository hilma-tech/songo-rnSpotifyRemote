...
const SpotifyContent: React.FC = () => {
  ...
  const pushParent = useCallback(
    async (nextParent: ContentItem) => {
      try {
        if (
          (nextParent.container && nextParent.children == undefined) ||
          ...
          console.log('Parentll ', [newParent, ...parentItems].length);
  ...
  const fetchItems = async () => {
    try {
      ...
      if (recommended) {
        retrieved = await remote.getRecommendedContentItems({
          type: 'default',
          flatten: false,
        });
      } else {
      ...
      // await selectContentItem(retrieved[(retrieved.length - 1)]);

      // GET 'LIBRARY'
      const loadedChildrenLibrary = await remote.getChildrenOfItem(
        retrieved[retrieved.length - 1],
      );

      // GET 'PLAYLISTS'
      await selectContentItem(loadedChildrenLibrary[0]);
      // console.log("length ",parentItems.length)
      // await selectContentItem(parentItems[0].children[0])
    } catch (err) {
      onError(err);
    }
  };
  ...
  const showItemActions = useCallback((item: ContentItem) => {
    ...
    const actionOptions = [
      ...
      // { text: 'Queue Uri', action: () => remote.queueUri(item.uri) },
      {
        text: 'Play Item',
        action: async () => {
          remote.playItem(item);
          const id = await setTrackID(item.id);
        },
      },
    ];

    if (
      item.uri.includes('spotify:playlist:') ||
      item.uri.includes('spotify:album:')
    ) {
      actionOptions.push({
    ...
    ActionSheet.show(
      {
        ...
        cancelButtonIndex: actionOptions.findIndex(
          ({text}) => text === cancelOption.text,
        ),
        title: item.title,
      },
      async (index) => {
  ...
  const selectContentItem = useCallback(
    async (item: ContentItem) => {
      if (item.uri.includes('spotify:playlist:')) {
        const chosenItem = await setChosenPlayListItem(item);
        console.log('item OF PLAYLIST IN CONTENT: ', chosenItem);
      }
